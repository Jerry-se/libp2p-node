...
use libp2p::{identity, swarm::NetworkBehaviour, Multiaddr, multiaddr::Protocol, PeerId};
...
// const IPFS_PROTO_NAME: StreamProtocol = StreamProtocol::new("/jerry9999mars/kad/1.0.0");
...
impl Behaviour {
    pub(crate) fn new(
        ...
        let kademlia = if enable_kademlia {
            ...
            // kademlia_config.set_protocol_names(iter::once(IPFS_PROTO_NAME).collect());
            ...
            // }
            for bpn in &bootstrap_nodes {
                let addr = Multiaddr::from_str(bpn).unwrap();
                match addr.iter().last() {
                    Some(Protocol::P2p(peer_id)) => {
                        kademlia.add_address(&peer_id, addr.clone());
                    },
                    _ => {
                        println!("Failed to extract peer id from: {:?}", bpn);
                    }
                };
            }
